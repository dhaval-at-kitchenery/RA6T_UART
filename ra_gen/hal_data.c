...
const timer_cfg_t g_timer_cfg =
		{ .mode = TIMER_MODE_PWM,
				/* Actual period: 0.000008333333333333334 seconds. Actual duty: 50%. */.period_counts =
						(uint32_t) 0x3e8, .duty_cycle_counts = 0x1f4,
				.source_div = (timer_source_div_t) 0, .channel = 10,
				.p_callback = NULL,
...
const timer_instance_t g_timer = { .p_ctrl = &g_timer_ctrl, .p_cfg =
		&g_timer_cfg, .p_api = &g_timer_on_gpt };
...
/* Baud rate calculated with 0.009% error. */.semr_baudrate_bits_b.abcse = 0,
		...
		.cks = 0, .brr = 43, .mddr = (uint8_t) 173, .semr_baudrate_bits_b.brme =
				true };

/** UART extended configuration for UARTonSCI HAL driver */
const sci_uart_extended_cfg_t g_uart_cfg_extend = { .clock = SCI_UART_CLOCK_INT,
		.rx_edge_start = SCI_UART_START_BIT_FALLING_EDGE, .noise_cancel =
				...
				&g_uart_baud_setting, .flow_control = SCI_UART_FLOW_CONTROL_RTS,
#if 0xFF != 0xFF
...
const uart_cfg_t g_uart_cfg = { .channel = 0, .data_bits = UART_DATA_BITS_8,
...
/* Instance structure to use this module. */
const uart_instance_t g_uart = { .p_ctrl = &g_uart_ctrl, .p_cfg = &g_uart_cfg,
		.p_api = &g_uart_on_sci };
